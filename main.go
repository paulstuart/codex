...
// TODO: makes this build tag enabled
...
func main() {
	...
	g.Printf("// generated by 'codex %s'; DO NOT EDIT\n", strings.Join(os.Args[1:], " "))
	g.Printf("\npackage %s\n", g.pkg.name)
...
type Generator struct {
	...
	pkg *PkgInfo     // Package we are scanning.
}
...
type File struct {
	src  string
	size int64       // just to see how it shows
	blah compact.F16 // test import
	pkg  *PkgInfo    // Package to which this file belongs.
...
func (g *Generator) parsePackage(directory string, names []string, text interface{}) {
	...
	for _, name := range names {
		...
		debugf("parsing file: %q", name)
		b, err := os.ReadFile(name)
		if err != nil {
			log.Fatalf("read error: %v", err)
		}

		parsedFile, err := parser.ParseFile(fs, name, b, 0)
		// debugf("PF %+v", parsedFile)
		if err != nil && name != "db_generated.go" {
			log.Fatalf("parsing package: %s: %s", name, err)
		}
		// src := fs.Position(parsedFile.Pos()).String()
		// src := fs.Position(0).String()
		// parsedFile.
		// debugf("POS:%+v", parsedFile.Pos())
		src := string(b)
...
func (pkg *PkgInfo) check(fs *token.FileSet, astFiles []*ast.File) {
...
func pstruct(typeName string, fields *ast.FieldList) *SQLInfo {
	debugf("PSTRUCT: %s -- %+v", typeName, fields)
	info := SQLInfo{
		Fields:     make(map[string]string), // [memberName]sqlName
		FieldNames: make([]string, len(fields.List)),
		FieldTypes: make([]string, len(fields.List)),
		Order:      make([]string, 0, len(fields.List)),
		NoUpdate:   make(map[string]struct{}),
	}
	good := false
	for i, field := range fields.List {
		//name := strings.Join(field.Names[0].Name, "::")
		name := field.Names[0].Name
		info.FieldNames[i] = name
		// ftype := field.Type
		// info.FieldTypes[i] = [fields.ftype.Pos():ftype.End()]//field
		// info.FieldNames = append(info.FieldNames, )
		if t := field.Tag; t != nil {
			s := strings.Trim(string(t.Value), "`")
			// the code uses backticks to metaquote, need to strip them whilst evaluating
			tag := reflect.StructTag(s)
			if sql := tag.Get(*tagKey); len(sql) > 0 {
				//fmt.Println("SQL:", sql)
...
//
//
func processStruct(typeName string, fields *ast.FieldList) *SQLInfo {
	info := SQLInfo{
		Fields:     make(map[string]string), // [memberName]sqlName
		FieldNames: make([]string, len(fields.List)),
		FieldTypes: make([]string, len(fields.List)),
...
func (f *File) genDecl(node ast.Node) bool {
	// if len(f.src) > 32 {
	// 	log.Println("SRC:", f.src[:32])
	// }
	// log.Printf("GENDECL: %T", node)
	switch x := node.(type) {
	case *ast.TypeSpec:
		f.TypeName = x.Name.Name
		log.Printf("TYPEDEF: %s", f.TypeName)
		return true
	case *ast.StructType:
		if f.TypeName != "" {
			f.structInfo(x)
			/*
				log.Printf("XSTRUCT: %s -- %+v", f.TypeName, x)
				// ff := make([]string, 0, len(f.))
				if len(f.findName) == 0 || f.findName == f.TypeName {
					if tags := processStruct(f.TypeName, x.Fields); tags != nil {
...
func (f *File) structInfo(stype *ast.StructType) {
	list := stype.Fields.List
	fields := make([]string, len(list))
	for i, item := range list {
		if len(item.Names) > 0 {
			fields[i] = item.Names[0].Name
		}
		kind := f.src[item.Type.Pos()-1 : item.Type.End()]
		// log.Printf("POS: %d", item.Type.Pos())
		log.Printf("%s::%s", fields[i], kind)
	}
	show := strings.Join(fields, ",")

	log.Printf("X_STRUCT: %s -- %v", f.TypeName, show)
}
...
func (g *Generator) buildWrappers(s *SQLInfo) {
	...
	for i, k := range s.Order {
		if len(k) > 0 {
			v := s.Fields[k]
			name := s.FieldNames[i]
			ftype := s.FieldTypes[i]
			debugf("field: %s\t%s", name, ftype)
			sql = append(sql, v)
...
func parseFile(filename string) error {
	...
	file, err := parser.ParseFile(fset, filename, f, parser.ImportsOnly)
	if err != nil {
		return fmt.Errorf("parser fail: %w", err)
	}
...
type SInfo struct {
	Field   string
	Type    string
	AltSize string // text for size handler
}

// TODO: build an AST and render it
// TODO: determine name used struct for methods
func buildDecoder(w io.Writer, name string, fields ...SInfo) error {
	const (
		entry = `func (xx %s).Decode(b []byte) error {
		var idx int
		`
		line = `if err := (%s).Decode(b[idx:]; err != nil {
			return err
			}
			idx += %s
			`
	...
	fmt.Fprintf(w, entry, name)
	for _, field := range fields {
		size := field.AltSize
		if size == "" {
			size = fmt.Sprintf("%s.Size()", field)
		}
		fmt.Printf(line, field, field)
	}
